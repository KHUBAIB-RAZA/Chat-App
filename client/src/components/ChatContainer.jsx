  ...
  return selectedUser ? (
    <div className='h-full overflow-y-auto relative backdrop-blur-lg'>
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'> 
        ...
        <img onClick={() => setselectedUser(null)} src={assets.arrow_icon} alt="" className='w-6 h-6 sm:w-8 sm:h-8 cursor-pointer md:hidden' /> </div>
        ...
        {messages.map((msg, index) => (
          ...
          ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
            ...
            <div className="text-center text-xs">
              <img 
              src={msg.senderId === authUser._id ? authUser?.
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          ...
          <input onChange={handleSendImage} type="file" id='image' accept='image/png, image/jpeg' hidden /> <label htmlFor="image">
