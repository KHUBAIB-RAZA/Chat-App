...
const ProfilePage = () => {
  ...
  return (
    <div className="min-h-screen bg-cover bg-no-repeat flex items-center justify-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-10 flex-1">
          ...
          <label htmlFor="avatar" className="flex items-center gap-3 cursor-pointer">
            <input onChange={(e)=>setSelectedImg(e.target.files[0])} type="file"id="avatar" accept=".png, .jpg, .jpeg" hidden /> 
              <img src={selectedImg? URL.createObjectURL(selectedImg): authUser?.profilePic ? authUser.profilePic: assets.avatar_icon }alt="" className={`w-12 h-12 ${selectedImg &&
              'rounded-full'}`}/>Edit profile image
      </label>
      ...
      <button type="submit" className="bg-gradient-to-r from-purple-400 to-violet-600 text-white p-2 rounded-full text-1g cursor-pointer">
        Save</ button>
